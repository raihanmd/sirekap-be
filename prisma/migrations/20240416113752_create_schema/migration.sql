-- CreateTable
CREATE TABLE `candidates_type` (
    `name` VARCHAR(50) NOT NULL,

    UNIQUE INDEX `candidates_type_name_key`(`name`),
    PRIMARY KEY (`name`)
...
-- AddForeignKey
ALTER TABLE `candidates` ADD CONSTRAINT `candidates_type_fkey` FOREIGN KEY (`type`) REFERENCES `candidates_type`(`name`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `voting_events` ADD CONSTRAINT `voting_events_type_fkey` FOREIGN KEY (`type`) REFERENCES `candidates_type`(`name`) ON DELETE RESTRICT ON UPDATE CASCADE;
